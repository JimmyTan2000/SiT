...
## Training SiT
Step 1: Run the following command (for a single GPU setup)
```bash 
torchrun --nnodes=1 --nproc_per_node=1 train.py --model SiT-B/8 --num-classes 1 --epochs 1000 --global-batch-size 150 --num-workers 1 --ckpt-every 2000 --cfg-scale 1.0 --image-size 128
```
Note: The dataset is contained in the `datasets` directory and it is generated using [this repository](https://github.com/JimmyTan2000/nerf-image-generation).
...
```bash
torchrun --nnodes=1 --nproc_per_node=1 train.py --model SiT-B/8 --data-path /path/to/imagenet/train --path-type Linear --prediction noise
```
- `Resume training:`To resume training from custom checkpoint:
```bash
torchrun --nnodes=1 --nproc_per_node=1 train.py --model SiT-B/8 --data-path /path/to/imagenet/train --ckpt /path/to/model.pt
```
